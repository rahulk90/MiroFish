"""
Ontology生成服务
接口1：分析文本内容，生成适合社会Simulation的Entity和关系类型定义
"""
...
# Ontology生成的系统提示词
ONTOLOGY_SYSTEM_PROMPT = """你是一个专业的知识GraphOntology设计专家。你的Task是分析给定的文本内容和Simulation需求，设计适合**社交媒体舆论Simulation**的Entity类型和关系类型。
...
- 每个Entity都是一个可以在社交媒体上发声、互动、传播信息的"账号"或"主体"
- Entity之间会相互影响、转发、评论、回应
- 我们需要Simulation舆论事件中各方的反应和信息传播路径
    ...
    "edge_types": [
        {
            ...
            "description": "简短描述（英文，不超过100字符）",
            "source_targets": [
                {"source": "源Entity类型", "target": "目标Entity类型"}
            ],
            "attributes": []
        }
    ],
...
**数量要求：必须正好10个Entity类型**

**层次结构要求（必须同时包含具体类型和兜底类型）**：

你的10个Entity类型必须包含以下层次：
...
- 数量：6-10个
- 关系应该反映社媒互动中的真实联系
- 确保关系的 source_targets 涵盖你定义的Entity类型
...
class OntologyGenerator:
    ...
    def generate(
        ...
        Args:
            ...
            simulation_requirement: Simulation需求描述
    ...
    def _build_user_message(
        ...
        # 如果文本超过5万字，截断（仅影响传给LLM的内容，不影响Graph构建）
        if len(combined_text) > self.MAX_TEXT_LENGTH_FOR_LLM:
            combined_text = combined_text[:self.MAX_TEXT_LENGTH_FOR_LLM]
            combined_text += f"\n\n...(原文共{original_length}字，已截取前{self.MAX_TEXT_LENGTH_FOR_LLM}字用于Ontology分析)..."
...
"""
        ...
        message += """
请根据以上内容，设计适合社会舆论Simulation的Entity类型和关系类型。

**必须遵守的规则**：
1. 必须正好输出10个Entity类型
2. 最后2个必须是兜底类型：Person（个人兜底）和 Organization（组织兜底）
3. 前8个是根据文本内容设计的具体类型
4. 所有Entity类型必须是现实中可以发声的主体，不能是抽象概念
...
"""
    ...
    def _validate_and_process(self, result: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # Zep API 限制：最多 10 个自定义Entity类型，最多 10 个自定义Edge类型
        MAX_ENTITY_TYPES = 10
        MAX_EDGE_TYPES = 10
