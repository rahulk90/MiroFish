...
# Adding this prefix avoids Agent tool calls and ensures text-based responses
INTERVIEW_PROMPT_PREFIX = "Based on your personality, all past memories and actions, respond directly with text without calling any tools:"
...
def get_graph_entities(graph_id: str):
    ...
    Returns only nodes conforming to predefined entity types (nodes with Labels other than Entity are excluded)
    ...
    try:
        ...
        logger.info(f"Retrieving knowledge graph entities: graph_id={graph_id}, entity_types={entity_types}, enrich={enrich}")
        ...
        logger.error(f"Failed to retrieve knowledge graph entities: {str(e)}")
        return jsonify({
            "success": False,
            "error": str(e),
...
def create_simulation():
    ...
    Note: Parameters like max_rounds are intelligently generated by LLM and do not need manual configuration
    ...
    try:
        ...
        if not graph_id:
            return jsonify({
                "success": False,
                "error": "Project has not built a knowledge graph yet. Please call /api/graph/build first"
            }), 400
...
def _check_simulation_prepared(simulation_id: str) -> tuple:
    ...
    Note: Run scripts (run_*.py) are kept in backend/scripts/ directory and are not copied to simulation directory
...
def prepare_simulation():
    ...
    - If preparation is complete, returns existing results directly
    ...
    try:
        ...
        # This allows frontend to immediately get expected Agent total count after calling prepare
        try:
            logger.info(f"Synchronously retrieving entity count: graph_id={state.graph_id}")
            reader = ZepEntityReader()
            # Quickly read entities (no need for edge information, just count)
            filtered_preview = reader.filter_defined_entities(
                graph_id=state.graph_id,
                defined_entity_types=entity_types_list,
                enrich_with_edges=False  # No edge information needed, faster
            )
            ...
            # Failure doesn't affect subsequent process, background task will retry
        ...
        def run_prepare():
            try:
                ...
                def progress_callback(stage, progress, message, **kwargs):
                    ...
                    stage_names = {
                        "reading": "Reading knowledge graph entities",
...
def get_prepare_status():
    ...
    1. Query running task progress by task_id
    2. Check if preparation work is already complete by simulation_id
...
def _get_report_id_for_simulation(simulation_id: str) -> str:
    ...
    and return the newest one (sorted by created_at) if multiple exist
    ...
    try:
        ...
        # Sort by creation time in reverse order, return the latest
...
def get_simulation_history():
    ...
    try:
        ...
        logger.error(f"Failed to retrieve simulation history: {str(e)}")
        return jsonify({
            "success": False,
            "error": str(e),
            "traceback": traceback.format_exc()
...
def get_simulation_profiles_realtime(simulation_id: str):
    """
    Real-time retrieval of Agent Profile (to view progress during generation)
    ...
    try:
        ...
        # Check if generating (determine from state.json)
        is_generating = False
        total_expected = None
...
def get_simulation_config_realtime(simulation_id: str):
    """
    Real-time retrieval of simulation configuration (to view progress during generation)
    ...
    try:
        ...
        logger.error(f"Real-time configuration retrieval failed: {str(e)}")
        return jsonify({
            "success": False,
            "error": str(e),
            "traceback": traceback.format_exc()
...
def get_simulation_config(simulation_id: str):
    ...
    Returns:
        - time_config: Time configuration (simulation duration, rounds, peak/low hours)
        - agent_configs: Activity configuration for each Agent (activity level, speaking frequency, stance, etc.)
...
# ============== Profile Generation Interfaces (Standalone) ==============
...
# ============== Simulation Execution Control Interfaces ==============
...
def start_simulation():
    ...
    About force parameter:
        ...
        - Suitable for scenarios needing to re-run simulations

    About enable_graph_memory_update:
        - When enabled, all Agent activities (posting, commenting, liking, etc.) in simulation are real-time updated to Zep knowledge graph
        - This allows knowledge graph to "remember" the simulation process for subsequent analysis or AI conversations
        - Requires the simulation-associated project to have valid graph_id
    ...
    Returns:
        {
            ...
            "data": {
                ...
                "graph_memory_update_enabled": true,  // Whether knowledge graph memory update is enabled
                "force_restarted": true               // Whether force restart occurred
            }
        }
    """
    try:
        ...
        # Intelligent status handling: allow restart if preparation is complete
        if state.status != SimulationStatus.READY:
            # Check if preparation is complete
            is_prepared, prepare_info = _check_simulation_prepared(simulation_id)

            if is_prepared:
                # Preparation complete, check for running process
        ...
        if enable_graph_memory_update:
            ...
            if not graph_id:
                return jsonify({
                    "success": False,
                    "error": "Enabling knowledge graph memory update requires valid graph_id. Please ensure project has built the knowledge graph"
                }), 400

            logger.info(f"Enabling knowledge graph memory update: simulation_id={simulation_id}, graph_id={graph_id}")
...
def get_run_status_detail(simulation_id: str):
    ...
    try:
        ...
        # recent_actions only shows current latest round from both platforms
...
def interview_agent():
    ...
    Note: This feature requires simulation environment to be running (in wait-for-commands mode after completing simulation loop)
    ...
    try:
        ...
        if not SimulationRunner.check_env_alive(simulation_id):
            return jsonify({
                "success": False,
                "error": "Simulation environment is not running or has closed. Please ensure simulation is complete and in wait-for-commands mode."
            }), 400
    ...
    Request (JSON):
        {
            ...
            "platform": "reddit",              // Optional, default platform (overridden by each item's platform)
                                               // If not specified: dual-platform simulation interviews both platforms for each Agent
...
def interview_all_agents():
    ...
    Request (JSON):
        {
            "simulation_id": "sim_xxxx",            // Required, simulation ID
            "prompt": "What is your overall perspective on this?",  // Required, interview question (all Agents use same question)
...
def close_simulation_env():
    ...
    Send close environment command to simulation to gracefully exit wait-for-commands mode.

    Note: This is different from /stop endpoint. /stop forcefully terminates the process,
    while this endpoint allows simulation to gracefully close environment and exit.
