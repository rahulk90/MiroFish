<template>
  ...
  >
    ...
    <div class="section-header">
      ...
      <span class="section-title">Inference History</span>
      <div class="section-line"></div>
    </div>
    ...
    <div v-if="projects.length > 0" class="cards-container" :class="{ expanded: isExpanded }" :style="containerStyle">
      ...
      >
        ...
        <!-- Card title (use first 20 chars of simulation requirement) -->
      ...
      <Transition name="modal">
        <div v-if="selectedProject" class="modal-overlay" @click.self="closeModal">
          <div class="modal-content">
            ...
            <!-- Inference playback divider -->
            <div class="modal-divider">
              <span class="divider-line"></span>
              <span class="divider-text">Inference Playback</span>
              <span class="divider-line"></span>
            </div>
            ...
            <div class="modal-playback-hint">
              <span class="hint-text">Step3 "Start Simulation" and Step5 "Deep Interaction" must be started during execution; history playback is not supported</span>
            </div>
          </div>
        </div>
      </Transition>
...
let pendingState = null  // Tracks target state to be executed
...
const containerStyle = computed(() => {
  ...
  // Calculate required height: number of rows * card height + (rows - 1) * gap + small bottom margin
...
const getCardStyle = (index) => {
  ...
  if (isExpanded.value) {
    ...
    // Calculate number of cards in current row to ensure centered alignment
    ...
    // Collapsed state: fan-like stacking
    const transition = 'transform 700ms cubic-bezier(0.23, 1, 0.32, 1), opacity 700ms cubic-bezier(0.23, 1, 0.32, 1), box-shadow 0.3s ease, border-color 0.3s ease'
  ...
  observer = new IntersectionObserver(
    (entries) => {
      entries.forEach((entry) => {
        ...
        // Update target state to be executed (record latest target state regardless of animation state)
        pendingState = shouldExpand

        // Clear previous debounce timer (new scroll intent will override old ones)
        if (expandDebounceTimer) {
          clearTimeout(expandDebounceTimer)
          expandDebounceTimer = null
        }

        // If animation is in progress, only record state and process after animation ends
        ...
        expandDebounceTimer = setTimeout(() => {
          ...
          // Check if pending state still needs execution (may be overridden by subsequent scrolling)
          ...
          setTimeout(() => {
            ...
            // After animation ends, check if there's new pending state
            if (pendingState !== null && pendingState !== isExpanded.value) {
              // Delay slightly before execution to avoid too rapid switching
              expandDebounceTimer = setTimeout(() => {
                if (pendingState !== null && pendingState !== isExpanded.value) {
                  isAnimating = true
                  isExpanded.value = pendingState
...
// Watch for route changes, reload data when returning to home page
