    ...
    <div class="graph-container" ref="graphContainer">
      ...
      <div v-if="graphData" class="graph-view">
        ...
        <div v-if="currentPhase === 1 || isSimulating" class="graph-building-hint">
          ...
          {{ isSimulating ? 'GraphRAG short/long-term memory updating in real-time' : 'Real-time updating...' }}
        </div>
        ...
        <div v-if="showSimulationFinishedHint" class="graph-building-hint finished-hint">
          ...
          <span class="hint-text">Some content is still being processed, recommend manually refreshing the graph later</span>
...
const renderGraph = () => {
  ...
  // Record which edge index we're at for each node pair
  const edgePairIndex = {}
  ...
  tempEdges.forEach(e => {
    ...
    let curvature = 0
    if (totalCount > 1) {
      // Evenly distributed curvature, ensure clear differentiation
      ...
      // Ensures all edges distribute under same reference frame, no overlap from direction differences
      if (isReversed) {
        curvature = -curvature
      }
    }
  ...
  const getLinkPath = (d) => {
    ...
    // Offset perpendicular to connection line, calculated by distance ratio, ensures visible curve
    ...
    .call(d3.drag()
      ...
      .on('drag', (event, d) => {
        ...
        if (!d._isDragging && distance > 3) {
          // First detection of true dragging, restart simulation
          d._isDragging = true
  ...
  // Close detail panel on blank area click
  svg.on('click', () => {
    selectedItem.value = null
    node.attr('stroke', '#fff').attr('stroke-width', 2.5)
    linkGroup.selectAll('path').attr('stroke', '#C0C0C0').attr('stroke-width', 1.5)
