...
Implements ReACT pattern for simulation report generation using LangChain + Zep
...
class ReportLogger:
    ...
    Generates agent_log.jsonl file in report folder, recording detailed actions at each step.
    ...
    def log_tool_result(
        ...
        """Log tool call result (complete content, no truncation)"""
        self.log(
            action="tool_result",
            stage="generating",
            section_title=section_title,
    ...
    def log_llm_response(
        ...
        """Log LLM response (complete content, no truncation)"""
        self.log(
            action="llm_response",
            stage="generating",
            section_title=section_title,
    ...
    def log_section_content(
        ...
        """Log section content generation complete (only logs content, does not represent full section completion)"""
        self.log(
    ...
    def log_section_full_complete(
        ...
        Frontend should listen to this log to determine if a section is fully complete and get complete content
        """
        self.log(
...
class ReportConsoleLogger:
    ...
    These logs are different from agent_log.jsonl, formatted as plain text console output.
    """
...
2. Retrieve information from simulation knowledge graph from multiple dimensions
...
- Need to understand multiple aspects of events
- Need to get rich material to support report sections
...
【Panorama Search - Get Overall View】
...
3. Help you understand how opinions evolved

【Use Cases】
- Need to understand the complete development context of events
...
- Need to verify certain facts
- Simple information retrieval

【Return Content】
- List of facts most related to the query"""
...
This is not LLM simulation, but calling real interview interface to get original answers from simulation Agents.
By default, interviews are conducted simultaneously on both Twitter and Reddit platforms to gather more comprehensive perspectives.

Functional Workflow:
1. Automatically read personality profile files to understand all simulated Agents
...
4. Call /api/simulation/interview/batch API to conduct real interviews on dual platforms
5. Integrate all interview results to provide multi-perspective analysis

【Use Cases】
- Need to understand different stakeholder perspectives on events (How do students view it? How does media view it? What do officials say?)
...
PLAN_SYSTEM_PROMPT = """\
You are an expert in writing \"Future Prediction Reports\" with a \"god-like perspective\" of the simulation world—you can understand the behavior, remarks, and interactions of every Agent in the simulation.

【Core Concept】
We built a simulation world and injected specific \"simulation requirements\" as variables into it. The evolution of the simulation world is a prediction of what might happen in the future. What you are observing is not \"experimental data,\" but a \"preview of the future.\"
...
3. What future trends and risks are worth paying attention to based on this simulation?

【Report Positioning】
- ✅ This is a future prediction report based on simulation, revealing \"what would happen if...\"
- ✅ Focus on prediction results: event trajectory, group responses, emergent phenomena, potential risks
...
- ❌ Not a generic summary of public sentiment
...
- Design section structure based on prediction results

Output report outline in JSON format as follows:
{
    "title": "Report Title",
...
Review this future preview from a \"god-like perspective\":
1. What state did the future present under the conditions we set?
2. How did various populations (Agents) react and act?
3. What future trends are worth paying attention to based on this simulation?

Design the most appropriate report section structure based on prediction results.

【Reminder】Report section count: minimum 2, maximum 5, content should be concise and focus on core prediction findings."""

# ── Section generation prompt ──

SECTION_SYSTEM_PROMPT_TEMPLATE = """\
You are an expert in writing \"Future Prediction Reports\" and are writing one section of the report.
...
1. 【Must call tools to observe the simulation world】
   - You are observing a preview of the future from a \"god-like perspective\"
   ...
   - Call tools at least 3 times (maximum 5) per section to observe the simulated world representing the future
...
2. 【Must quote original Agent words and actions】
   ...
   - Use quote format in the report to show these predictions, for example:
     > "A certain population will state: original text..."
...
3. 【Language Consistency - Quoted content must be translated to report language】
   - Tool returns may contain English or mixed English-Chinese expressions
   - If simulation requirements and source materials are in Chinese, the entire report must be in Chinese
   - When quoting tool-returned English or mixed content, must translate it to fluent Chinese before writing into report
   - Keep original meaning when translating, ensure natural expression
...
4. 【Faithfully present prediction results】
   ...
   - If certain information is insufficient, state truthfully
...
- ❌ Forbidden to use any Markdown titles (#, ##, ###, #### etc.) in sections
...
- ✅ Use **bold text**, paragraph breaks, quotes, lists to organize content, but no titles

【Correct Example】
```
This section analyzes the propaganda dissemination of the event. Through in-depth analysis of simulation data, we found...

**Initial Viral Stage**

Weibo as the first scene of public sentiment played the core function of information initiation:

> "Weibo contributed 68% of initial voice..."

**Emotion Amplification Stage**

TikTok further amplified the event impact:
...
### 1. Initial Stage          ← Wrong! Don't use ### for subsections
#### 1.1 Detailed Analysis   ← Wrong! Don't use #### for details
...
- insight_forge: Deep insight analysis, automatically break down problems and retrieve facts and relationships from multiple dimensions
...
When you have obtained sufficient information through tools, start with \"Final Answer:\" and output section content.
...
3. Use Markdown format (but forbidden to use titles):
   - Use **bold text** to mark key points (instead of subsection titles)
   - Use lists (- or 1.2.3.) to organize points
   - Use blank lines to separate different paragraphs
   - ❌ Forbidden to use #, ##, ###, #### or any title syntax
4. 【Quote Format Specification - Must be standalone paragraph】
   ...
   > "The school's response pattern seemed rigid and slow in a rapidly changing social media environment."
...
6. 【Avoid Repetition】Carefully read completed sections below to avoid repeating the same information
7. 【Again Emphasize】Don't add any titles! Use **bold text** instead of subsection titles"""
...
2. Must call tools to get simulation data before you begin
...
- ❌ Don't write any titles (#, ##, ###, #### won't work)
- ❌ Don't write "{section_title}" as the opening
- ✅ Section titles are automatically added by the system
- ✅ Write body content directly, use **bold text** instead of subsection titles

Begin:
1. First think (Thought) about what information this section needs
2. Then call tools (Action) to get simulation data
3. After collecting sufficient information, output Final Answer (pure body text, no titles)"""

# ── Messages in ReACT loop template ──
...
- If information is sufficient: Start with "Final Answer:" and output section content (must cite original text above)
...
REACT_INSUFFICIENT_TOOLS_MSG_ALT = (
    "Currently only called {tool_calls_count} tools, need at least {min_tool_calls}. "
    "Please call tools to get simulation data. {unused_hint}"
)

REACT_TOOL_LIMIT_MSG = (
    "Tool call limit reached ({tool_calls_count}/{max_tool_calls}), cannot call tools again. "
    'Please immediately output section content starting with "Final Answer:" based on information obtained.'
)

REACT_UNUSED_TOOLS_HINT = "\n💡 You haven't used: {unused_list}, suggest trying different tools for multi-perspective information"
...
【Answer Style】
- Concise and direct, no long-winded discussions
...
class ReportAgent:
    ...
    2. Generation Phase: Generate content section by section, can call tools multiple times per section to get information
    ...
    def __init__(
        ...
        # Logger (initialized in generate_report method)
        self.report_logger: Optional[ReportLogger] = None
        # Console logger (initialized in generate_report method)
    ...
    def _execute_tool(self, tool_name: str, parameters: Dict[str, Any], report_context: str = "") -> str:
        ...
        try:
            if tool_name == "insight_forge":
                ...
                # Breadth search - Get overall view
                query = parameters.get("query", "")
            ...
            # ========== Backward compatible old tools (internally redirected to new tools) ==========
                ...
                logger.info("search_graph already redirected to quick_search")
                ...
                logger.info("get_simulation_context already redirected to insight_forge")
    ...
    # Set of legal tool names, used for validation when parsing raw JSON fallback
    ...
    def _parse_tool_calls(self, response: str) -> List[Dict[str, Any]]:
        ...
        # Only try when format 1 is not matched, to avoid mistakenly matching JSON in body
