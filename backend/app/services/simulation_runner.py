...
class SimulationRunState:
    ...
    # Independent round and simulation time for each platform (for dual-platform parallel display)
    ...
    def to_dict(self) -> Dict[str, Any]:
        return {
            ...
            # Independent round and time for each platform
...
class SimulationRunner:
    ...
    def start_simulation(
        ...
        try:
            ...
            # Create main log file to prevent stdout/stderr pipe buffer overflow
                        ...
                        try:
                            ...
                            if "event_type" in action_data:
                                ...
                                if event_type == "simulation_end":
                                    ...
                                    # If two platforms ran, both need to be completed
                            ...
                            # If graph memory update is enabled, send actions to Zep
                            if graph_updater:
    ...
    def _terminate_process(cls, process: subprocess.Popen, simulation_id: str, timeout: int = 10):
        ...
        Args:
            ...
            timeout:             timeout: Timeout for waiting process to exit (seconds)
        """
        if IS_WINDOWS:
    ...
    def _read_actions_from_file(
        ...
        """
        Read actions from individual action file
        
        Args:
                        file_path: Action log file path
                        default_platform: Default platform (when action record has no platform field)
    ...
    def get_all_actions(
        ...
        # Read Twitter action file (automatically set platform to twitter by file path)
        twitter_actions_log = os.path.join(sim_dir, "twitter", "actions.jsonl")
        ...
        # Read Reddit action file (automatically set platform to reddit by file path)
        reddit_actions_log = os.path.join(sim_dir, "reddit", "actions.jsonl")
    ...
    def cleanup_simulation_logs(cls, simulation_id: str) -> Dict[str, Any]:
        """
        Clean up simulation logs (to force restart simulation)
        ...
        # Clean up action logs in platform directory
    ...
    def cleanup_all_simulations(cls):
        ...
        Called when server shuts down, ensures all subprocesses are terminated
        """
        ...
        # Check if there is anything to clean up (avoid useless logs from empty processes)
        has_processes = bool(cls._processes)
        has_updaters = bool(cls._graph_memory_enabled)
        
        if not has_processes and not has_updaters:
            return  # No cleanup needed, return silently
    ...
    def register_cleanup(cls):
        ...
        # In Flask debug mode, only register cleanup in reloader subprocess (process actually running the app)
        # WERKZEUG_RUN_MAIN=true indicates it is a reloader subprocess
        # If not debug mode, this environment variable does not exist, registration is still needed
        ...
        def cleanup_handler(signum=None, frame=None):
            ...
            if signum == signal.SIGINT and callable(original_sigint):
                ...
                # If original handler is not callable (such as SIG_DFL), use default behavior
        ...
        Args:
            ...
            agent_id: Agent ID
                        ...
                        platform: Specify platform (optional)
                                ...
                                - None: for dual-platform simulation, interview both platforms simultaneously, return integrated result
        ...
        Args:
            simulation_id: Simulation ID
                        ...
                        platform: Default platform (optional, will be overridden by each interview item's platform)
                                ...
                                - None: for dual-platform simulation, interview both platforms simultaneously for each Agent
                        timeout: Timeout (seconds)

        Returns:
            批量            Interview result dictionary
    ...
    def interview_all_agents(
        ...
        Args:
            simulation_id: Simulation ID
                        prompt: Interview question（所有Agent使用相同问题）
                        platform: Specify platform (optional)
                                - "twitter": only interview Twitter platform
                                - "reddit": only interview Reddit platform
                                - None: for dual-platform simulation, interview both platforms simultaneously for each Agent
        ...
        Returns:
            全局            Interview result dictionary
        """
        sim_dir = os.path.join(cls.RUN_STATE_DIR, simulation_id)
        if not os.path.exists(sim_dir):
            raise ValueError(f"Simulation does not exist: {simulation_id}")
    ...
    def close_simulation_env(
        ...
        try:
            ...
            return {
                "success": True,
                "message": "Close environment command sent (waiting for response timeout, environment may be closing)"
            }
    ...
    def get_interview_history(
        ...
        # If querying multiple platforms, limit total number
